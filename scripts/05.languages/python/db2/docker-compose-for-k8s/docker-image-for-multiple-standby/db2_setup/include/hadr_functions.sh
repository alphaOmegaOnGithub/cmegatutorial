...
update_HADR_dbcfg_params() {
    ...
    grep -q -E "db2_hadrp|db2_hadrs|db2_hadra|db2_hadrb" /etc/services || { logger_error "$HADR_MISSING_SVCE_NAMES" && RC=1 && return $RC; }

    ### Update the HADR db configuration
    logger_info "Updating HADR database configuration parameters on BLUDB ..."
if [ "${REPLICAS?}" = "2" ] && [ "${role?}" = "primary" -o "${role?}" = "standby" ]; then
...
if [ "${REPLICAS?}" = "3" ]; then
    ...
    if [ "${role?}" = "primary" -o "${role?}" = "standby" ];then	
        cat <<EOF > /tmp/hadr_db.cfg
        LOGINDEXBUILD ON
        HADR_LOCAL_HOST $lhost
        HADR_LOCAL_SVC $lsvc
        ...
        HADR_SYNCMODE NEARSYNC
        HADR_PEER_WINDOW 120
        HADR_TARGET_LIST $rhost:$rsvc|$standby1_hostname:db2_hadra
EOF
    elif [ "${role?}" = "standby1" ];then
        cat <<EOF > /tmp/hadr_db.cfg
        LOGINDEXBUILD ON
        HADR_LOCAL_HOST $lhost
        ...
        HADR_PEER_WINDOW 120
        HADR_TARGET_LIST $rhost:$rsvc|$standby_hostname:db2_hadrs
EOF
    fi	    
fi
...
if [ "${REPLICAS?}" = "4" ]; then
    ...
    if [ "${role?}" = "primary" -o "${role?}" = "standby" ];then
        ...
        HADR_SYNCMODE NEARSYNC
        HADR_PEER_WINDOW 120
        HADR_TARGET_LIST $rhost:$rsvc|$standby1_hostname:db2_hadra|$standby2_hostname:db2_hadrb
EOF
    elif [ "${role?}" = "standby1" ];then
        cat <<EOF > /tmp/hadr_db.cfg
        LOGINDEXBUILD ON
        ...
        HADR_SYNCMODE SUPERASYNC
        HADR_PEER_WINDOW 120
        HADR_TARGET_LIST $rhost:$rsvc|$standby_hostname:db2_hadrs|$standby2_hostname:db2_hadrb
EOF
    elif [ "${role?}" = "standby2" ];then
        cat <<EOF > /tmp/hadr_db.cfg
        LOGINDEXBUILD ON
        HADR_LOCAL_HOST $lhost
        ...
        HADR_SYNCMODE SUPERASYNC
        HADR_PEER_WINDOW 120
        HADR_TARGET_LIST $rhost:$rsvc|$standby_hostname:db2_hadrs|$standby1_hostname:db2_hadra
EOF
    fi
fi
