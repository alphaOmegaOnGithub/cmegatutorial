...
while test $# -gt 0; do
    case $1 in
        ...
        ;;
	-standby1) current_node="standby1"
        ;;
	-standby2) current_node="standby2"
        ;;
...
if [ ["${current_node?}" == "standby"] || ["${current_node?}" == "standby1"] || ["${current_node?}" == "standby2"] ] && [ "${standby_init_method?}" == "backup" ]; then
...
if [ "${REPLICAS?}" = "3" ]; then
    update_etc_hosts "${remote_node_short_hostname?}" "${remote_node_ip?}" "${domain?}"
    if [ "${current_node?}" = "standby1" ]; then
        standby_hostname=$(${DB2DIR?}/bin/db2fupdt -f ${HADR_SHARED_DIR?}/hadr.cfg -p standby_hostname)
        standby_ip=$(${DB2DIR?}/bin/db2fupdt -f ${HADR_SHARED_DIR?}/hadr.cfg -p standby_ipaddr)
        standby_node_name=$(cut -d '.' -f 1 <<< "${standby_hostname?}")
	update_etc_hosts "${standby_node_name?}" "${standby_ip?}" "${domain?}"
    else
...
if [ "${REPLICAS?}" = "4" ]; then
    ...
    elif [ "${current_node?}" = "standby2" ]; then
        standby_hostname=$(${DB2DIR?}/bin/db2fupdt -f ${HADR_SHARED_DIR?}/hadr.cfg -p standby_hostname)
        standby_ip=$(${DB2DIR?}/bin/db2fupdt -f ${HADR_SHARED_DIR?}/hadr.cfg -p standby_ipaddr)
        standby_node_name=$(cut -d '.' -f 1 <<< "${standby_hostname?}")
        update_etc_hosts "${standby_node_name?}" "${standby_ip?}" "${domain?}"
...
case "${current_node?}" in
    primary)
        ...
        update_HADR_dbcfg_params "${remote_node_name?}" "db2_hadrp" "db2_hadrs" ${database?} ${instance?} "primary" || exit ${FALSE}

	# Set standby readable
	su - ${instance?} -c "db2set DB2_HADR_ROS=ON"
	su - ${instance?} -c "db2set DB2_STANDBY_ISO=UR"
    ...
    standby)
        ...
        update_HADR_dbcfg_params ${remote_node_name?} "db2_hadrs" "db2_hadrp" ${database?} ${instance?} "standby" || exit ${FALSE}
    ...
    standby1)
        ...
        logger_info "Update HADR database configuration parameters on ${current_node?} $(hostname -s)..."
        update_HADR_dbcfg_params ${remote_node_name?} "db2_hadrs" "db2_hadrp" ${database?} ${instance?} "standby1" || exit ${FALSE}
    ...
    standby2)
        ...
        logger_info "Update HADR database configuration parameters on ${current_node?} $(hostname -s)..."
        update_HADR_dbcfg_params ${remote_node_name?} "db2_hadrs" "db2_hadrp" ${database?} ${instance?} "standby2" || exit ${FALSE}
...
if [ "${current_node?}" == "standby" ] || [ "${current_node?}" == "standby1" ] || [ "${current_node?}" == "standby2" ]; then
